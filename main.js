...
const backup = require('./backupGenerator.js');
...
function firstBackupAndStartInterval(_hoursInterval) {
	try {
		...
		let error = 'error in firstBackupAndStartInterval' + '\n' + e.message + '\n' + e.stack;
		console.log(error);
...
try {
	...
	let error = 'error starting or error within main event loop. attempting to write to error log' + 
		e.message + '\n' + e.stack;
	console.log(error);
